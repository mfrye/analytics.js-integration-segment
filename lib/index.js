...
Segment.prototype.ontrack = function(track) {
  ...
  json = buildPayload(json, dataConfig);
...
function buildPayload(json, dataConfig) {

  // Set the event to the configured property
  if (dataConfig) {
    json.event = json.properties[dataConfig.event];
    delete json.properties.event;

    // Map fields to props if dataConfig exists
    for (var key in dataConfig) {
      // Check if the property exists
      if (key !== 'event' && json.properties[dataConfig[key]]) {
        json.properties[key] = json.properties[dataConfig[key]];
      }
    }
  }
